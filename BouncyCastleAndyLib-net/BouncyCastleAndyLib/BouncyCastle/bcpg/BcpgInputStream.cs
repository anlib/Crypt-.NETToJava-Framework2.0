...
//				return 0;
        ...
        private byte[] mReadAllBuf = new byte[128];
        public byte[] ReadAll()
        {
            MemoryStream buffer = new MemoryStream();

            int len;
            while ((len = this.Read(mReadAllBuf, 0, mReadAllBuf.Length)) > 0)
            {
                buffer.Write(mReadAllBuf, 0, len);
            }
            ...
            int		len)
        {
            while (len > 0)
            {
                int l = m_in.Read(buffer, off, len);
		...
		/// <summary>A stream that overlays our input stream, allowing the user to only read a segment of it.</summary>

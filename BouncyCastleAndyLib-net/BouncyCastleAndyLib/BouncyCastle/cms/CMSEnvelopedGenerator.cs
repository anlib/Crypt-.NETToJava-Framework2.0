...
namespace Org.BouncyCastle.Cms
{
	...
	*/
	public class CmsEnvelopedGenerator
	{
		...
		internal IList recipientInfs = new ArrayList();
		internal SecureRandom rand = new SecureRandom();
...
//					byte[] iv = keyWrapper.GetIV();
		...
		*/
		...
		{
			Pbkdf2Params p = new Pbkdf2Params(pbeKey.GetSalt(), pbeKey.IterationCount);
			KeyParameter secretKey = ParameterUtilities.CreateKeyParameter(
				kekAlgorithmOid, pbeKey.GetEncoded(kekAlgorithmOid));
		...
		*/
			...
			string					cekWrapAlgorithm)
		{
			...
			try
			{
				SubjectPublicKeyInfo oPubKeyInfo = SubjectPublicKeyInfoFactory.CreateSubjectPublicKeyInfo(senderPublicKey);
				...
				KeyParameter secret = ParameterUtilities.CreateKeyParameter(
					cekWrapAlgorithm, secretNum.ToByteArrayUnsigned());

				recipientInfs.Add(new RecipientInf(cekWrapAlgorithm, secret, agreementAlgorithm, cekWrapAlgorithm, originator, recipientCert));
			}
			catch (IOException e)
			{

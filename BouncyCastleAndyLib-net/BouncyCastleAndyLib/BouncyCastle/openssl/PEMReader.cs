...
//					DerInteger v = (DerInteger)seq[0];
			...
			string	dekInfo)
		{
			...
			string encoding = tknz[0].Trim();
			byte[] iv = Hex.Decode(tknz[1].Trim());

			ICipherParameters sKey;
			string cipherName;

			if (encoding.Equals("DES-EDE3-CBC"))
			{
				sKey = GetKey("DESede", 24, iv);
				cipherName = "DESede/CBC/PKCS5Padding";
			}
			else if (encoding.Equals("DES-CBC"))
			{
				sKey = GetKey("DES", 8, iv);
				cipherName = "DES/CBC/PKCS5Padding";
			}
			else
			{
			...
			IBufferedCipher c = CipherUtilities.GetCipher(cipherName);
			c.Init(false, new ParametersWithIV(sKey, iv));
